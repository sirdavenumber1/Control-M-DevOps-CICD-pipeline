...
echo $tmp | sed -i '/^*\"Command\".*/ s/C/CCCCCCCC/g' > xtemp_job_file.json 
...
######curl -k -s -H "Authorization: Bearer $token" -X POST -F "definitionsFile=@ctmjobs/jobs.json" -F "deployDescriptorFile=@ctmjobs/DeployDescriptorPROD.json" "$ENDPOINT/deploy" 
